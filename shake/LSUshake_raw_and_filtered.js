...
//var chanCode = 'HHZ';
var netCode = 'AM';
//var staCodes = 'R95B0,RB0B5,R38D9';
// Nicholson
//var staCodes = 'R95B0';
// Lee High School
//var staCodes = 'REB59';
...
//var HOSTscedc = 'service.scedc.caltech.edu';
//var HOSTShake = 'fdsnws.raspberryshakedata.com';
var HOSTShake ='data.raspberryshake.org'
var protocolShake = 'https:';
...
console.log("plotOneStation: "+mystation.codes());
    ...
    return new fdsndataselect.DataSelectQuery(HOSTShake)
      .protocol(protocolShake)
      ...
      .then(dataRecords => {
        ...
        for (var key of byChannel.keys()) {
          var segments = wp.miniseed.merge(byChannel.get(key));
            ...
            div.append('p').html('Filtered seismogram for ' + key);
            var svgdiv = div.append('div').attr('class', 'myseisplot');
			
			div.append('p').html('Raw data for ' + key);
            var svgdivraw = div.append('div').attr('class', 'myseisplot');

			 if (segments.length > 0) {

         for(let i=0; i<segments.length; i++) {
           segments[i] = seisplotjs_filter.rMean(segments[i]);
         }
         console.log("Sample rate: "+segments[0].sampleRate());
         var seismographRaw = new wp.Seismograph(svgdivraw, segments, startDate, endDate);
         seismographRaw.draw();
                 // filtering
                         let butterworth = seisplotjs_filter.createButterworth(
                          ...
                          0, // low corner
                          0.6, // high corner
                          1.0/segments[0].sampleRate() // delta (period)
                 );
                 var filteredSeismogram = [];
                 for(let i=0; i<segments.length; i++) {
                           console.log(" seis "+segments[i].codes());
                                   for (var spseg in segments[i]) {
									console.log("prop "+spseg);}
                                   var traces = segments[i].clone();
                                           console.log(" clone "+traces.codes());
                                           console.log(" clone "+traces.chanCode().charAt(2));

                   butterworth.filterInPlace(traces.y());
                   filteredSeismogram.push(traces);
                 }

                 var seismograph1 = new wp.Seismograph(svgdiv, filteredSeismogram, startDate, endDate);
                               var markers = [];
                 markers.push({ markertype: 'predicted', name: "origin", time: quake.time() });
                 markers.push({ markertype: 'predicted', name: firstPS.firstP.phase, time:moment.utc(quake.time()).add(firstPS.firstP.time,'second') });
                 markers.push({ markertype: 'predicted', name:firstPS.firstS.phase, time:moment.utc(quake.time()).add(firstPS.firstS.time, 'second') });
               ...
               seismograph1.appendMarkers(markers);
               seismographRaw.appendMarkers(markers);
               seismograph1.draw();
           }
